...
static err_t qn_linkoutput( struct netif * netif,
                            struct pbuf * p )
{
    ...
    sent = sendto( s->fd, buf, p->tot_len, 0,
...
static void qn_rx_task( void * arg )
{
    ...
    for( ; ; )
    {
        ...
        while( ( n = recvfrom( s->fd, buf, sizeof( buf ),
                               MSG_DONTWAIT, NULL, NULL ) ) > 0 )
        {
            ...
            /* netif->input is tcpip_input(): thread-safe hand-off to tcpip. */
