		      ...
		      const ip_addr_t *addr)
{
	...
	if ((p->tot_len < hlen + sizeof(iecho)) ||
	    (pbuf_copy_partial(p, &iecho, sizeof(iecho), hlen) !=
	     sizeof(iecho))) {
		return 0;
	}
...
static err_t ping_send(const ip_addr_t *dst, u16_t seq)
{
	struct pbuf *p;
	struct icmp_echo_hdr *iecho;
