...
static int pattern_value( enum pattern p, unsigned seq )
{
    switch( p )
    {
        case PAT_SINE:   return ( int ) ( 128.0 + 100.0 *
                                          sin( TWO_PI * ( seq % SINE_PERIOD ) / SINE_PERIOD ) );
        case PAT_RANDOM: return rand() & 0xff;
        case PAT_CONST:  return 42;
        default:         return ( int ) ( seq & 0xff );  /* ramp */
    }
}
...
{
    mqtt_publish( s_client, s_topic_resp, msg, ( u16_t ) strlen( msg ),
                  0, 0, NULL, NULL );
}
...
static void handle_command( char * cmd )
{
    ...
    else if( strncmp( cmd, "rate ", 5 ) == 0 )
    {
        ...
        {
            snprintf( resp, sizeof resp, "error: rate out of range (10..60000)" );
        }
        publish_resp( resp );
    }
    ...
    {
        const char * a = cmd + 8;
        if( strcmp( a, "ramp" ) == 0 )        { s_pattern = PAT_RAMP; }
        else if( strcmp( a, "sine" ) == 0 )   { s_pattern = PAT_SINE; }
        else if( strcmp( a, "random" ) == 0 ) { s_pattern = PAT_RANDOM; }
        else if( strcmp( a, "const" ) == 0 )  { s_pattern = PAT_CONST; }
        else { publish_resp( "error: unknown pattern" ); return; }
        snprintf( resp, sizeof resp, "pattern=%s", pattern_name( s_pattern ) );
        publish_resp( resp );
    }
    else if( strncmp( cmd, "led ", 4 ) == 0 )
    {
        if( strcmp( cmd + 4, "on" ) == 0 )       { s_led = 1; }
        else if( strcmp( cmd + 4, "off" ) == 0 ) { s_led = 0; }
        else { publish_resp( "error: led expects on|off" ); return; }
    ...
    for( ; ; )
    {
        if( !s_connected )
        {
            ...
            printf( "mqtt: connecting to %s:%u as \"%s\"\n",
                    ipaddr_ntoa( &s_broker_ip ), s_broker_port, s_clientid );

            LOCK_TCPIP_CORE();
            e = mqtt_client_connect( s_client, &s_broker_ip, s_broker_port,
                                     connection_cb, NULL, &s_ci );
            UNLOCK_TCPIP_CORE();

            if( e != ERR_OK )
            {
                printf( "mqtt: connect call failed (%d), retrying\n", e );
            }
...

        {
            ...
            int n = snprintf( payload, sizeof payload,
                              "{\"seq\":%u,\"val\":%d,\"pattern\":\"%s\",\"led\":%d}",
                              seq, val, pattern_name( s_pattern ), s_led );
            err_t e;

            LOCK_TCPIP_CORE();
            e = mqtt_publish( s_client, s_topic_tel, payload, ( u16_t ) n,
                              0, 0, NULL, NULL );
            UNLOCK_TCPIP_CORE();
            ...
            else if( ( e == ERR_CONN ) || !mqtt_client_is_connected( s_client ) )
            {
                s_connected = 0;
            }
            seq++;
...
void mqtt_app_start( const struct app_cfg * cfg )
{
    ...
    snprintf( s_topic_tel, sizeof s_topic_tel, "dev/%s/telemetry", cfg->hostname );
    snprintf( s_topic_cmd, sizeof s_topic_cmd, "dev/%s/cmd", cfg->hostname );
    snprintf( s_topic_resp, sizeof s_topic_resp, "dev/%s/resp", cfg->hostname );
