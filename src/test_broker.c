...
static void send_publish( const char * topic, const char * payload )
{
    ...
    size_t rl = 2 + tlen + plen;          /* topic-len field + topic + payload (QoS0) */
    int i = 0, rlbytes;
...
static void handle_packet( uint8_t type, uint8_t flags,
...
static void parse_packets( void )
{
    ...
    while( ( s_rxlen - off ) >= 2 )
    {
        ...
        while( i < s_rxlen )
        {
            ...
            if( nbytes >= 4 )   { s_rxlen = 0; return; } /* malformed */
        }
...
static void cmd_task( void * arg )
{
    static const char * cmds[] = {
        "status", "rate 500", "pattern sine", "led on",
        "ping", "pattern random", "led off", "status"
    };
    char topic[ 64 ];
    LWIP_UNUSED_ARG( arg );
