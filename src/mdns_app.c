...
void mdns_app_start( const struct app_cfg * cfg )
{
    ...
    /* Only advertise the broker if we'll actually be using it; with --no-mqtt
     * the static buffer stays empty and srv_txt skips the mqtt= TXT. */
    if( !cfg->no_mqtt )
    {
        snprintf( s_broker, sizeof s_broker, "%s:%d",
    ...
    if( err == ERR_OK )
    {
        slot = mdns_resp_add_service( nif, s_hostname, "_drone",
                                      DNSSD_PROTO_TCP,
                                      ( u16_t ) cfg->broker_port,
                                      srv_txt, NULL );
        if( slot < 0 )
        {
            /* Keep responder state consistent: the netif was registered but
             * the service slot wasn't, so a later retry would otherwise trip
             * lwIP's 'Double add' assert. */
